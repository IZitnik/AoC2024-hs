...
solve m b = foldl go S.empty m
  where go :: S.Set Anode -> [Antenna] -> S.Set Anode
        go s a = S.union s (S.fromList $ findAntiFreqs a b)

findAntiFreqs :: [Antenna] -> Bounds -> [Anode]
findAntiFreqs an b = foldl go [] an
  where go :: [Anode] -> Antenna -> [Anode]
        go a c = (concat [takeWhile (inBounds b) (getAnode c x) | x<-an, x/=c]) ++ a

getAnode :: Antenna -> Antenna -> [Anode]
getAnode (sx,sy) (dx,dy) = let mx = (sx-dx)
                               my = (sy-dy)
                            in [(sx+mx*i, sy+my*i) | i<-[0..]]
